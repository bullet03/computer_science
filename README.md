...
- По умолчанию результаты работы immer (produce) фризятся, результаты вложенных produce не фризятся. Имеются возможности настройки того поведения (setAutoFrize)
...
- при работе с custom classes Immer создаеьт новый this, подтягивает старый prototype
- для exotic objects, Data, ... сложные случаи, Immer говорит не надеяться на него, а испоьлзовать return/не использовать Immer вообще
- искусстенно freeze Map - вручную перебить его методы (wrapper/monkey patching)
