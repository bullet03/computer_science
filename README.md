...
- cache/memoization - cache (получение данных не из источника, наппример, copy); memoization (частный случай cache для ускорения получения результата, для предотвращения повторного запуска функции)
...
- input selector не должен всегда возвращать новый результат (в случае redux весь state), иначе теряется смысл оптимизириующей логики HOS (selectors высшего порядка), в частнмо случае мемоизации
- output selector не имеет смысла, если он не производит рансформирующую логику. Легче сразу заьрать данные от низшего selector без создания HOS (selectors высшего порядка), т.е. без использования re-select
